...
void TIM3_IRQHandler()	 //定时器3中断函数 FlagBeepState
{
	static uint16_t TimeLength = 0;
	static uint16_t Flag_IO = 0;
	static uint16_t didit = 0;
	...
	if(FlagBeepState == 1)
	{
			...
			else if((TimeLength >= 1200) && (TimeLength < 1800))
			{
				GPIO_ResetBits(GPIOB, GPIO_Pin_5);
			}
			else
