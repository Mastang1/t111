...
extern uint8_t CSR_Pos[];		//¹â±êÎ»ÖÃÉèÖÃÖ¸Áî£¬ºóÐø·¢ËÍÊý¾Ý
...
/*==========´®¿Ú1¿ØÖÆÒÔÌ«ÍøÄ£¿éÏà¹ØÊý×é===================*/
uint8_t Usart_ON[]={0x2a,0x2b,0x2c};									//´ò¿ª´®ÐÐÄ£Ê½Ö¸Áî
uint8_t Usart_OFF[]={0x3e,0x57,0x42,0x52,0x31,0x0d};	//¹Ø±Õ´®ÐÐÄ£Ê½Ö¸Áî
...
/*=================¼ÌµçÆ÷Ö±Í¨==============*/
	for(j=0;j<6;j++)																			//¹Ø±ÕËùÓÐµÄ¼ÌµçÆ÷
	{
		Relay_OFF (0x0400<<j);									//LVDTÖÐÊÇPF10-PF15,ÆäÖÐ0x400±íÊ¾PF10£¬Ñ­»·×óÒÆµ½PF15
		delay_ms(50);
	}

///*=================Ä£Äâ¿ª¹Ø±ÕºÏÊ¹µçÈÝ·Åµç£¨PF10-PF15£©===============*/
//	Analog_sw_ON (0xFC00);
...
/*=================AD5292¸´Î»¿ØÖÆ===============*/	
	for(i=0;i<6;i++)
	{
		...
		SPI_R=Write_CMD(0x0001<<i,0x1803);								//ËùÓÐÍ¨µÀµÄAD5292¸´Î»(CS from PE0-PE5)
...
*           									 ÊµÏÖÏµÍ³×Ô¼ì
*******************************************************************************/
void AUTO_Check(void)	//ÏµÍ³×Ô¼ìÖ´ÐÐ³ÌÐò
{
	uint8_t j;
	uint8_t i;
	/*=================¼ÌµçÆ÷Ö±Í¨==============*/
...
/*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&´Ë´¦Òª×¢Òâ¶Ë¿ÚÊÇ·ñ¶ÔÓ¦&&&&&&&&&&&&&&&&&&&&&&&&&&&*/				
			...
			{			
				...
				LCD_Comand(Clear_LCD);
				LCD_display(channel,LCD_Display_CMD,16,0,0);    		 //¹ÊÕÏ×¢ÈëµÚÒ»ÐÐÒº¾§ÏÔÊ¾
				
				Current_channel=Buffer_last ^ RxBuffer1[2];					//Òì»ò
				Buffer_last=RxBuffer1[2];
				
				for(i=0;i<6;i++)																		//forÑ­»·£¬ÂÖÑ¯6¸öÍ¨µÀµÄÊý¾Ý
				{
					if(Current_channel & (0x40>>i))										//ÁùÎ»Êý¾Ý£¬´Ó×óµ½ÓÒ·Ö±ðÊÇ1-6Í¨µÀ
					{
							if(RxBuffer1[2] & (0x40>>i))
							{
								uint8_t Rx;																	//¶¨Òå½ÓÊÕµ½µÄ¸ßÎ»Öµ£¨RxBuffer1[2i+3]µÄµÍËÄÎ»£©
								uint16_t Set_unit;													//¶¨ÒåSet_unit´æ·Å¸öÎ»Êý
								uint8_t Set_dec;														//¶¨ÒåSet_dec´æ·ÅÐ¡Êý²¿·Ö
								uint16_t Set_Value;
								uint16_t Digipot;
								float ratio;
...
//								}
		/*************************************************************************/						
								...
								SPI_R=Write_CMD(0x0001<<i,0x1803);					//´ò¿ª¶ÔÓ¦Í¨µÀµÄÐ´Èë£¨Ã»ÎÊÌâ£©
								SPI_R++;																
		...
		/*************************ÊäÈëÖµ¼ÆËã**********************£¿£¿£¿£¿£¿£¿£¿**********************/							
								...
								Souce_L_bit=RxBuffer1[(i*2)+17];					//»ñÈ¡Ð¡Êý²¿·ÖµÄÖµ						
								Souce_Value=(uint16_t)(Souce_H_bit+Souce_L_bit);											//´ËÊ±µÄSet_ValueÖµÊÇ¸öÈýÎ»ÊýÕûÊý°
									
									
		/****************************Êä³öÖµ¼ÆËã***************************************************/
								Rx=(RxBuffer1[(i*2)+3]) & 0x3f;	//°Ñ¸ßÎ»µÄ6¸öÎ»µÄÖµ¸³Öµ¸øRx0
								Set_unit=(uint16_t)(Rx*100);		//¸öÎ»³ËÒÔ100,³ýÒÔ2ËùµÃ¼´ÎªÊä³öÎªVPP
								Set_dec=RxBuffer1[(i*2)+4];					//»ñÈ¡Ð¡Êý²¿·ÖµÄÖµ
								Set_Value=(uint16_t)(Set_unit+Set_dec);											//´ËÊ±µÄSet_ValueÖµÊÇ¸öÈýÎ»ÊýÕûÊý

								ratio=((float)Set_Value)/((float)Souce_Value)*14;				//14K,´ËÊ±Îªµç×èÖµ

																			
								ratio=1-ratio/20.00;							//ratioÊÇ¸¡µãÐÍÊý¾Ý	
								ratio=ratio*1024;								//´ËÊ±µÄratioÊÇRDAC¼Ä´æÆ÷ÐèÒªµÄÖµ£¨¸¡µãÐÍ£© 
								Digipot=(uint16_t)(ratio+0.5);	//´ËÊ±µÄDigipotÊÇÊý×ÖµçÎ»Æ÷Ó¦¸ÃÐ´ÈëµÄÖµ,Ó¦ÓÃËÄÉáÎåÈë
									if(Digipot>1023) Digipot=1023;
		/*=====================Ð´Êý×ÖµçÎ»Æ÷============================================*/
																																	//ÐÎ³ÉÖ¸Áî
								SPI_R=Write_CMD(0x01<<i,0x0400|Digipot);		//¾­²âÊÔ£¬0x0500Ê±ºò£¬ÊýÖµÎª15K£»RAW½Ó·¨£¬
		/*=====================Õý¸ºÖµÅÐ¶Ï²¢ÉèÖÃ============================================*/	
...
Description:Ö¸ÁîÖ´ÐÐº¯Êý,µ±»ñÈ¡ÍêÕûÊý¾Ý°üºó£¬¼ÆËãÎÂ¶È£¬È»ºóÅÐ¶ÏÊÇ·ñ³¬¹ýãÐÖµ£¬½ø¶ø¿ØÖÆÖ¸Ê¾µÆµÄ×´Ì¬
...
void USART2CMD_exe(void)
{
	if(RECEIVE_flag2==SET)//RECEIVE_flag2
	{
			...
			{
					GPIO_SetBits(GPIOA, GPIO_Pin_1);																				 //ºìÉ«Ö¸Ê¾µÆÁÁ£¨LVDTÖÐµÄÖ¸Ê¾µÆ¿ØÖÆÎ»ÊÇPF7Òý½Å£©¼´P4 Òý½Å
			}
