...
#include "public.h"
/*变量定义--------------------------------------------------------------------------------*/
unsigned int flag_YN = 1;
unsigned int flag_RSuc = 0;
unsigned int test_YN = 1;
...
//判断是否为帧头		
			...
			else if( (buffer != 0xAA)  &&  (buffer != 0x55) && (uart2Cnt < 2))
			{
				flag_YN = 0;
				RxBuffer2N[uart2Cnt] = buffer;
				uart2Cnt++;
			}else
			{
				if(flag_YN)
				{
					RxBuffer2[uart2Cnt] = buffer;
				}
				else
				{
					RxBuffer2N[uart2Cnt] = buffer;
